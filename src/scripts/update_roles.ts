...
export async function removePermissionsField() {
  try {
    // Update all documents in the Role collection to remove the permissions field
    const result = await RoleModel.updateMany(
      {},
      { $unset: { permissions: '' } },
    );
    console.log(
      `Permissions field removed from ${result.modifiedCount} roles.`,
    );
  } catch (error) {
    console.error('Error removing permissions field:', error);
  }
}
