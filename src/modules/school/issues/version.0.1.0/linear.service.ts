...
class LinearService {
  ...
  private async updateIssueWithReply(
  ...
  private async getIssueById(
    ...
    const issue = await LinearIssueModel.findById(id).session(session).exec();
    if (!issue) {
      throw new Error('No issue found');
    }
    return issue;
  ...
  public async addAdminsReply(
    ...
    try {
      return await withTransaction(async (session) => {
        ...
        const reply = (await newReply.save({ session })) as LinearIssue;
        if (!reply) {
