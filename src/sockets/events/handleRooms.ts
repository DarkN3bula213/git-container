...
export const handleRooms = (socket: Socket) => {
  ...
  const joinRoom = ({ roomId, userId, userName }: IJoinRoomParams) => {
    ...
    logAndEmit(events.get_users, { roomId, participants: rooms[roomId] });
    socket.to(roomId).emit(events.users_joined, { userId, userName });
  ...
  const changeName = ({ roomId, userId, userName }: IJoinRoomParams) => {
    if (rooms[roomId] && rooms[roomId][userId]) {
      ...
      socket.to(roomId).emit(events.name_changed, { userId, userName });
    }
  };
